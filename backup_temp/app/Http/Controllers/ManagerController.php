...
use App\Mail\BeneficiaryInvite;
...
class ManagerController extends Controller
{
    ...
    public function inviteBeneficiary(Request $request, $id)
    {
        $manager = $request->user();
        $isAdmin = str_contains(strtolower((string) $manager->role), 'admin');
        ...
        $inviteUrl = rtrim(env('FRONTEND_URL', config('app.url')), '/') . '/beneficiary/invite/' . $token;

        try {
            Mail::to($validated['email'])->send(new BeneficiaryInvite($application, $validated['message'] ?? '', $manager, $inviteUrl));
        } catch (\Exception $e) {
            \Illuminate\Support\Facades\Log::error('Beneficiary invite email failed: ' . $e->getMessage());
            return response()->json(['message' => 'Invite saved but sending email failed.'], 500);
        }

        return response()->json([
            'message' => 'Beneficiary invite sent successfully.',
            'application' => $application->load(['user', 'manager']),
        ]);
    }
}
